...
func (r *Request) UnmarshalNamespace() (*corev1.Namespace, error) {
	fmt.Printf("Admission Request: %#v", r)
	if r.Admission == nil {
...
func CalculateAnnotations(ns *corev1.Namespace) ([]byte, error) {
		...
		if k == "externalsecrets.kubernetes-client.io/permitted-key-name" {
			...
			return json.Marshal(
				[]map[string]string{
					{
						"op":    "replace",
						"path":  "/metadata/annotations/externalsecrets.kubernetes-client.io~1permitted-key-name",
						"value": annotation,
					},
				},
			)
	...
	return json.Marshal(
		[]map[string]interface{}{
			{
				"op":    "add",
				"path":  fmt.Sprintf("/metadata/annotations"),
				"value": map[string]string{"externalsecrets.kubernetes-client.io/permitted-key-name": annotation},
			},
		},
	)
}
